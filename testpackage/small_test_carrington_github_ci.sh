...
for run in ${run_tests[*]}; do
  ...
  if [[ ${single_cell[$run]} ]]; then
    {
      $small_run_command $bin --run_config=${test_name[$run]}.cfg 2>&1 1>&3 3>&- | tee $GITHUB_WORKSPACE/stderr.txt
      exit ${PIPESTATUS[0]}
    } 3>&1 1>&2 | tee $GITHUB_WORKSPACE/stdout.txt
  else
    {
      $run_command $bin --run_config=${test_name[$run]}.cfg 2>&1 1>&3 3>&- | tee $GITHUB_WORKSPACE/stderr.txt
      exit ${PIPESTATUS[0]}
    } 3>&1 1>&2 | tee $GITHUB_WORKSPACE/stdout.txt
  fi
...

  {
    ...
    for vlsv in ${comparison_vlsv[$run]}; do
      ...
      for i in ${!variables[*]}; do
        ...
        elif [ ! "${variables[$i]}" == "proton" ]; then # Regular vg_ variable
    ...
    done 2>&1 1>&3 3>&- | tee -a $GITHUB_WORKSPACE/stderr.txt
